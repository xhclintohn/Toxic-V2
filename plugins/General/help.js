...
${body}\n╰───────────────\n> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 𝐱𝐡_𝐜𝐥𝐢𝐧𝐭𝐨𝐧`;
        ...
        if (!args || args.length === 0) {
            ...
            return await client.sendMessage(m.chat, {
                text: fmt('ALL COMMANDS', `│ Total: ${allCommands.length} commands\n│ Use *${effectivePrefix}help <command>* for full usage + description\n│ \n${body}`)
            });
        }
        ...
        const helpData = {
            ...
            listactive: { usage: `${effectivePrefix}listactive`, desc: 'Top 10 most active users in the group by message count (tags only). Aliases: activelist, aet, activeusers, topactive, active' },
            liststicker: { usage: `${effectivePrefix}liststicker`, desc: 'List sticker counts per user in this group (tags only). Aliases: sl, totalsticker, stickert, stickerlist, stickercount' },
            antilinkgc: { usage: `${effectivePrefix}antilinkgc <off|on|delete|warn|kick>`, desc: 'Block WhatsApp group/channel invite links only. on/delete = delete only. Aliases: antlinkgc, nolinkgc' },
            antilink: { usage: `${effectivePrefix}antilink <off|on|delete|warn|kick>`, desc: 'Block all links. on/delete = delete only, warn = delete+warn, kick = instant kick.' },
            antigroupstatus: { usage: `${effectivePrefix}antigroupstatus <off|on|delete|warn|kick>`, desc: 'Punish non-admins who post group status. on maps to delete.' },
            antisticker: { usage: `${effectivePrefix}antisticker <off|on|delete|warn|kick>`, desc: 'Block stickers. on/delete = delete only.' },
            stealth: { usage: `${effectivePrefix}stealth <on|off>`, desc: 'When ON, bot still responds but its own messages auto-delete after 3 seconds. DB-persisted.' },
            setwelcome: { usage: `${effectivePrefix}setwelcome image <url>|off`, desc: 'Set custom welcome image URL used when a member joins (requires welcome on).' },
            fancy: { usage: `${effectivePrefix}fancy <number> <text>`, desc: 'Convert text into a fancy unicode style (1 to available styles).' },
            gstatus: { usage: `${effectivePrefix}gstatus [all|link|jid] [caption]`, desc: 'Post media or text as group status. Reply to image/video/audio/text or provide caption. all = every group.' } };
