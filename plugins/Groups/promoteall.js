...
export default {
    ...
    run: async (context) => {
        await middleware(context, async () => {
            ...
            if (!m.isGroup) return client.sendMessage(m.chat, { text: BOX('ERROR', ['Group only command.']) });
            if (!isBotAdmin) return client.sendMessage(m.chat, { text: BOX('ERROR', ['Make me admin first.']) });
            ...
            try {
                ...
                if (!jids.length) return client.sendMessage(m.chat, { text: BOX('PROMOTEALL', ["Everyone's already an admin. Nothing to do."]) });

                await client.sendMessage(m.chat, { text: BOX('PROMOTEALL', [`Promoting ${jids.length} members...`]) });
                ...
                await client.sendMessage(m.chat, { react: { text: '❌', key: m.reactKey } });
                await client.sendMessage(m.chat, { text: BOX('ERROR', [`Failed: ${err.message?.slice(0, 60)}`]) });
            }
        });
    }
};
